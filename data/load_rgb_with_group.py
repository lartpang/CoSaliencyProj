...
class CoSAL_Sampler(data.Sampler):
    """
    Borrow from ICNet, NIPS2020
    """

    def __init__(self, idxes, shuffle, batch_size, drop_last, fill_batch=True):
        self.idxes = idxes
        ...
        self.len = None
        self.batches_idxes = None
        self.reset_batches_idxes()

    def get_batch_idx(self, total_idxes, batch_size):
        assert isinstance(batch_size, int), batch_size

        batch = []
        for idx in total_idxes:
            batch.append(idx)
        ...
        if len(batch) > 0 and not self.drop_last:
            if self.fill_batch:
                last_batch = total_idxes[-batch_size:]
            else:
                last_batch = batch
            yield last_batch

    def reset_batches_idxes(self):
        batches_idxes = []

        start_idx = 0
        # 对每个图片组(属于同一个类别):
        for end_idx in self.idxes:
            # 获取当前图片组的容量大小.
            num = end_idx - start_idx

            # 初始化 "group_idxes".
            group_idxes = list(range(start_idx, end_idx))

            if self.shuffle:
                # 打乱组内顺序.
                random.shuffle(group_idxes)

            # 根据 "self.batch_size" 将 "group_idxes" 划分为多个batches,
            # 然后将划分好的结果(即 "batch_idxes")添加到 "batches_idxes" 中.
            # 注意, 当 "self.batch_size == None" 时, 每个图片组都被直接作为一个batch (即 "batch_size = num").
            batch_size = num if self.batch_size is None else self.batch_size
            for batch_idxes in self.get_batch_idx(total_idxes=group_idxes, batch_size=batch_size):
                batches_idxes.append(batch_idxes)

            start_idx = end_idx

        if self.shuffle:
            # 打乱batch之间(组间)顺序
            random.shuffle(batches_idxes)

        self.len = len(batches_idxes)
        self.batches_idxes = batches_idxes

    def __iter__(self):
        if self.shuffle:
            self.reset_batches_idxes()
        return iter(self.batches_idxes)
