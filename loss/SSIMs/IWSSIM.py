...
class IW_SSIM():
    ...
    def test_iw_ssim(self, gray_imgo, gray_imgd):
        gray_imgo = gray_imgo.astype(self.samplen.dtype)
        gray_imgd = gray_imgd.astype(self.samplen.dtype)
        imgopr, imgdpr = self.get_pyrd(gray_imgo, gray_imgd)
    ...
    def test_iw_sparse(self, gray_imgo, gray_imgd):
        gray_imgo = gray_imgo.astype(self.samplen.dtype)
        gray_imgd = gray_imgd.astype(self.samplen.dtype)
        imgopr, imgdpr = self.get_pyrd(gray_imgo, gray_imgd)
        l_map, cs_map = self.scale_qualty_maps(imgopr, imgdpr)
